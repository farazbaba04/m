...
// Your Firebase API Key (from config)
...
async function loadUsers() {
    ...
    usersList.innerHTML = '<div class="card"><div class="loading">Loading users...</div></div>';
...
function createUserCard(user) {
    ...
    card.innerHTML = `
        ...
        ${user.role !== 'ADMIN' ? `<button class="delete-btn" onclick="deleteUser('${user.uid}')">🗑️ Delete</button>` : '<span style="color: #999; font-size: 12px;">👑 Admin User</span>'}
    `;
    return card;
}
...
async function deleteUser(uid) {
    ...
    try {
        ...
        showAlert(`✅ User "${userData.fullName}" deleted successfully!`, 'success');
        await loadUsers();
